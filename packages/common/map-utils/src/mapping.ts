...
type EliminateDuplicates<R extends RoTuple> =
  R extends HeadTail<R, infer Head, infer Tail>
  ? Head extends Tail[number]
    ? EliminateDuplicates<Tail>
    : [Head, ...EliminateDuplicates<Tail>]
  : R;
...
type ValsOfObjectUnion<T> = T[keyof T];
...
type WidenedParamsAndRet<M extends Mapped, D extends number> =
  [Widen<FromExtPropKey<keyof M>>, ...WidenedParamsAndRetRec<M, D>];
...
>(mapping: M, shape?: S): ToMapping<M, S> => {
  ...
  // tsc apparently can't track the mutation possibility here
