...
import type { Text, TODO, Url } from "@stable-io/utils";
...
>(
  ...
  const allQuotes = await cctprContract.quoteOnChainRelay(
  ...
  const costs = corridors.map((_, i) => {
    ...
    return [usdcCost!, gasCost!] as const;
  });
  return costs as RoArray<RelayCost<N, S>>; // @todo: remove cast
};
