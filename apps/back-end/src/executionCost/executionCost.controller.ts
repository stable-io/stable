...
export class ExecutionCostController {
  constructor(private readonly executionCostService: ExecutionCostService) {}
  ...
  public getExecutionCosts(
    @Param("platform") platform: string,
  ): ApiResponseDto<EvmExecutionCostDto> {
    const supportedPlatforms =
      this.executionCostService.getSupportedPlatforms();
    ...
    const result = this.executionCostService.getKnownEstimates(
    ...
    return {
      data: serializeBigints(result) as Record<
        keyof EvmExecutionCosts,
        SerializedBigint
      >,
    };
  }
